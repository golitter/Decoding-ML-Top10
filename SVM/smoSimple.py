from smo_assist import select_Jrandom, clip_alpha
	...
	while iter < max_iter:
		...
		for i in range(m):
			...
			if (
				...
				eta = (
					2.0 * data_mat_in[i, :] @ data_mat_in[j, :].T
					- data_mat_in[i, :] @ data_mat_in[i, :].T
					- data_mat_in[j, :] @ data_mat_in[j, :].T
				)
				"""
				...
				b1 = (
					b
					- Ei
					- class_labels[i] * (alphas[i] - alpha_i_old) * data_mat_in[i, :] @ data_mat_in[i, :].T
					- class_labels[j] * (alphas[j] - alpha_j_old) * data_mat_in[i, :] @ data_mat_in[j, :].T
				)
				b2 = (
					b
					- Ej
					- class_labels[i] * (alphas[i] - alpha_i_old) * data_mat_in[i, :] @ data_mat_in[j, :].T
					- class_labels[j] * (alphas[j] - alpha_j_old) * data_mat_in[j, :] @ data_mat_in[j, :].T
				)
				"""
                更新 b
                """
